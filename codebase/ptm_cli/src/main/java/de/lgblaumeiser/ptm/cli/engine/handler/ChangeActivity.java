...
public class ChangeActivity extends AbstractHandlerWithActivityRequest {
	...
	public void handleCommand() {
		...
		if (name != null) chgAct.setActivityName(name);
		if (identifier != null) chgAct.setBookingNumber(identifier);
		if (oldAct.isHidden() && visible) chgAct.setHidden(false);
		if (!oldAct.isHidden() && hidden) chgAct.setHidden(true);
