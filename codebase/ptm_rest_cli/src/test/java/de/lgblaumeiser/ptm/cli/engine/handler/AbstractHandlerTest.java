...
public abstract class AbstractHandlerTest {
	...
	public void before() {
		...
		// bookings.stream().filter(b -> b.getId() ==
		// object.getId()).findFirst().ifPresent(bookings::remove);
		// try {
		...
		// public void deleteById(Long id) {
		// bookings.stream().filter(b -> b.getId() ==
		// id).findFirst().ifPresent(bookings::remove);
		// }
		// });
		// services.setBookingService(new
		// BookingServiceImpl().setBookingStore(services.getBookingsStore()));
		// services.setAnalysisService(new DataAnalysisService() {
		// @Override
		// public Collection<Collection<Object>> analyze(String analyzerId,
