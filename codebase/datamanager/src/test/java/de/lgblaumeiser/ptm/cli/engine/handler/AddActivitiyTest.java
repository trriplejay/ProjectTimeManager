...
public class AddActivitiyTest extends AbstractHandlerTest {
	...
	@Test
	public void testAddActivityThreeParamLine() {
		testee.handleCommand(asList(ACTIVITY1NAME, ACTIVITY1NUMBER, "line"));
		Collection<Activity> results = services.getActivityService().getActivityModel().getActivities();
		assertEquals(1, results.size());
		assertEquals(ACTIVITY2, results.iterator().next());
		assertTrue(activityStoreCalled);
	}
	
	@Test
	public void testAddActivityThreeParamAnythingElse() {
		testee.handleCommand(asList(ACTIVITY1NAME, ACTIVITY1NUMBER, "bullshit"));
		Collection<Activity> results = services.getActivityService().getActivityModel().getActivities();
		assertEquals(1, results.size());
		assertEquals(ACTIVITY1, results.iterator().next());
		assertTrue(activityStoreCalled);
	}
